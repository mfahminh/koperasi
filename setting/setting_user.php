...
<script language="JavaScript">
	...
	function showName(show_kode_anggota){
		$.ajax({
			type : "POST",
			data : "show_kode_anggota="+show_kode_anggota,
			url : "dataAnggota.php",
			success : function(msg){
				hasil = jQuery.parseJSON(msg);
				if(hasil.NAMA_ANGGOTA!=""){
					$('#nama_anggota').val(hasil.NAMA_ANGGOTA);
				}else{		
					$('#nama_anggota').val("");
				}
			}
...
<div class="form-group" id="app">
  ...
  <select class="form-control" name="kode_anggota" id="show_kode_anggota" onChange="showName(this.value)" class="required">
  <?php
    $i=mysql_query("SELECT * from t_anggota WHERE addedUser = 0");
    while($data=mysql_fetch_array($i)){
  ?>
    <option value="<?php echo $data['kode_anggota'];?>"><?php echo $data['kode_anggota'];?></option> 
    <?php } ?>
  </select>
</div>
<div class="form-group">
<label>Nama</label>
<input name="nama" id="nama_anggota" class="form-control" type="text" class="required" readonly>
</div>
<div class="form-group">
