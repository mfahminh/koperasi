...
if($proses=='tolak')
{
	...
	$sql=mysql_query("UPDATE t_pengajuan SET status='ditolak' where kode_pengajuan='$kodep' and kode_anggota='$kodea'");
	if($sql)
	{ ?>
		<script>
...
else if($proses=='terima')
{
	...
	$kodea=$_GET['kode_anggota'];
	$date=date("Y-m-d");
	$sqla=mysql_query("UPDATE t_pengajuan set tgl_acc='$date',status='diterima' where kode_pengajuan='$kodep' and kode_anggota='$kodea'");
	$sqlb=mysql_fetch_array(mysql_query("SELECT * from t_pengajuan where kode_pengajuan='$kodep' and kode_anggota='$kodea'"));
	$anggota=$sqlb['kode_anggota'];
	$jenis=$sqlb['kode_jenis_pinjam'];
	$besarpin=$sqlb['besar_pinjam'];
	$oo=mysql_fetch_array(mysql_query("SELECT*from t_jenis_pinjam where kode_jenis_pinjam='$jenis'"));
    ...
    $sqlc=mysql_query("INSERT into t_pinjam values('','$anggota','$jenis','$besarpin','$total','$lama','','$besarpin','','$date','$tempo','belum lunas')");
    if($sqlc)
	{ ?>
		<script>
...
else if($proses=='hapus')
{
	$kodep=$_GET['kode_pengajuan'];
	$kodea=$_GET['kode_anggota'];
	$sql=mysql_query("DELETE from t_pengajuan where kode_pengajuan='$kodep' and kode_anggota='$kodea'");
	if($sql)
	{ ?>
		<script>
